...
def handle_advance(data):
    ...
    logger.info(f"Received update permission request info {data}")

    status = "accept"
    try:
        logger.info("Adding a permission update")
        response = requests.post(
            rollup_server + "/notice", json={"payload": data["payload"]})
        logger.info(
            f"Received notice status {response.status_code} with permission info: {response.content}")

    except Exception as e:
        status = "reject"
        msg = f"Error processing permission info {data}\n{traceback.format_exc()}"
        logger.error(msg)
        response = requests.post(
            rollup_server + "/report", json={"payload": str2hex(msg)})
        logger.info(
            f"Received report status {response.status_code} with permission info: {response.content}")

    return status


def handle_inspect(data):
    logger.info(f"Received inspect request info {data}")
